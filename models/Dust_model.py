...
class DedustModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_dust_J', type=float, default=1, help='weight for loss_dust_J')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_single', 'G_single', 'dust_J', 'idt_J']


        if self.isTrain:
            self.visual_names = ['real_I', 'refine_J', 'dust_J',
                                 'real_J', 'ref_real_J']
        else:
            self.visual_names = ['real_I', 'refine_J', 'dust_J']
        # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>.
        if self.isTrain:
            self.model_names = [ 'G_J', 'D']
        else:  # during test time, only load Gs
            self.model_names = [ 'G_J']
    ...
    def forward(self):
        ...
        self.dust_J = util.generate(self.real_I)
    ...
    def backward_G(self):
        ...
        lambda_dust_J = self.opt.lambda_dust_J
        ...
        # Reconstrcut loss
        self.loss_dust_J = self.criterionRec(self.real_I, self.dust_J) * lambda_dust_J

        # Identity loss
        self.ref_real_J = self.G_J(self.real_J)
        self.loss_idt_J = self.criterionIdt(self.ref_real_J, self.real_J) * lambda_idt
        self.loss_G = self.loss_G_single + self.loss_dust_J + self.loss_idt_J
